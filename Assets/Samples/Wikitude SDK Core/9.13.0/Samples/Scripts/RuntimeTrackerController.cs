...
public class RuntimeTrackerController : SampleController
{
    ...
    private IEnumerator LoadAssetBundle(SampleTrackerAddresses addresses) {
        string drawableAddress = addresses.assetBundleAndroid;
        if (Application.platform == RuntimePlatform.IPhonePlayer) {
            drawableAddress = addresses.assetBundleIOS;
        } else if (Application.platform == RuntimePlatform.WSAPlayerARM || Application.platform == RuntimePlatform.WSAPlayerX64 || Application.platform == RuntimePlatform.WSAPlayerX86) {
            drawableAddress = addresses.assetBundleUWP;
        }
