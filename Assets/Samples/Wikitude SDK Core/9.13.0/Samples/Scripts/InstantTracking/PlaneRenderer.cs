...
public class PlaneRenderer : MonoBehaviour {
    ...
    private readonly Dictionary<long, GameObject> _renderPlanes = new Dictionary<long, GameObject>();
    private readonly List<PlaneTarget> _trackedPlanes = new List<PlaneTarget>();

    public void OnPlaneRecognized(PlaneTarget recognizedPlane) {
        ...
        renderPlane.transform.localPosition = Vector3.zero;
        ...
        _renderPlanes.Add(recognizedPlane.TargetId, renderPlane);
        _trackedPlanes.Add(recognizedPlane);
    }

    public void Update() {
        foreach (var trackedPlane in _trackedPlanes) {
            if (_renderPlanes.TryGetValue(trackedPlane.TargetId, out var renderPlane)) {
                UpdateMesh(renderPlane, renderPlane.GetComponent<MeshFilter>().sharedMesh, trackedPlane);
            } else {
                Debug.LogError("Could not find tracked plane with ID: " + trackedPlane.TargetId);
            }
        }
    }

    public void OnPlaneLost(PlaneTarget lostPlane) {
        if (_renderPlanes.TryGetValue(lostPlane.TargetId, out var renderPlane)) {
            _renderPlanes.Remove(lostPlane.TargetId);
            Destroy(renderPlane);
        } else {
            Debug.LogError("Could not find lost plane with ID: " + lostPlane.TargetId);
        }

        _trackedPlanes.Remove(lostPlane);
    }

    private void UpdateMesh(GameObject renderPlane, Mesh mesh, PlaneTarget plane) {
        ...
        var convexHull = new List<Vector3>(vertexCount);
