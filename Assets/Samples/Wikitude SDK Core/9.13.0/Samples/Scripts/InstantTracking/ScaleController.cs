...
public class ScaleController : MonoBehaviour
{
    ...
    private void Update () {
        if (Input.touchCount >= 2) {
             ...
             */
            if (_activeObject != null) {
                /* Only proceed scaling if the tracking plane is available */
    ...
    private Vector3 GetGroundPosition(Vector2 touchPosition) {
        if (_controller.InstantTarget != null) {
            var groundPlane = new Plane(_controller.InstantTarget.transform.up, _controller.InstantTarget.transform.position);
            var touchRay = Camera.main.ScreenPointToRay(touchPosition);
            float enter;
